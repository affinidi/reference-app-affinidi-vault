...
const filesToIgnore = ['node_modules', '.next', '.env', 'generator-config.json', '.gitkeep', 'keys', 'appInformation.json']
...
async function generate() {
  ...
  const nextJsPath = join(generatorPath, 'nextjs')
  const overridesPath = join(nextJsPath, 'overrides')
  const templatePath = join(nextJsPath, 'template')
